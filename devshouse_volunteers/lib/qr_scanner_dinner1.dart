...
class QRScannerDinner1 extends StatefulWidget {
  const QRScannerDinner1({super.key});

  @override
  State<StatefulWidget> createState() => _QRScannerDinner1State();
}

class _QRScannerDinner1State extends State<QRScannerDinner1> {
  ...
  Future<void> _checkAndUpdateUser(String uniqueId) async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      ...
      if (doc['dinner_1'] != true) {
        await doc.reference.update({'dinner_1': true});
        _showDialog('Verified', Icons.check_circle_outline, Colors.green);
      } else {
        _showDialog(
            'Repeating Dinner - Don\'t Allow!', Icons.cancel, Colors.red);
      }
    } else {
